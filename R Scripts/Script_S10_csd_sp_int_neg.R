...
peak.list <- readRDS(gzcon(url("https://github.com/Pohnert-Lab/SC-MS-Identification/blob/master/RData_spectra/Spectra_S10_csd_sp_int_neg.RData?raw=true"))) # S10
...
for(i in 1:length(ref.peaklist)){
  ...
  top1result.part<-data.frame(real.identity  = paste(Algae$Genus[which(Algae$ID==ID)], Algae$Species[which(Algae$ID==ID)],sep = "_"),
                              ...
                              bt.species.eu.score = result$eu$species.score[1],
                              bt.species.ieu.score = result$ieu$species.score[1],
                              bt.species.cos.score = result$cosine$species.score[1],
...
reference<-unlist(sapply(as.character(top1result.df$ID), function(x) Algae$Species[which(Algae$ID == x)], simplify = FALSE),use.names = FALSE) 


## Format dataframe for ROC analysis
# Get Species names from metadata for top hit
tophits.Species <- unlist(sapply(as.character(top1result.df$ID), function(x) Algae$Species[which(Algae$ID == x)], simplify = FALSE),use.names = FALSE) 

# Create vector with booleans of Species hits
hits.eu <-  tophits.Species == unlist(sapply(as.character(top1result.df$sample.eu), function(x) Algae$Species[which(Algae$ID == x)], simplify = FALSE),use.names = FALSE)
hits.ieu <- tophits.Species == unlist(sapply(as.character(top1result.df$sample.ieu), function(x) Algae$Species[which(Algae$ID == x)], simplify = FALSE),use.names = FALSE)
hits.cos <- tophits.Species == unlist(sapply(as.character(top1result.df$sample.cos), function(x) Algae$Species[which(Algae$ID == x)], simplify = FALSE),use.names = FALSE)

# Create vector with names of Species hits
species.eu  <-unlist(sapply(as.character(top1result.df$sample.eu), function(x) Algae$Species[which(Algae$ID == x)], simplify = FALSE),use.names = FALSE)
species.ieu <-unlist(sapply(as.character(top1result.df$sample.ieu), function(x) Algae$Species[which(Algae$ID == x)], simplify = FALSE),use.names = FALSE)
species.cos <-unlist(sapply(as.character(top1result.df$sample.cos), function(x) Algae$Species[which(Algae$ID == x)], simplify = FALSE),use.names = FALSE)


# Combine booleans/Species names with top1result.df
top1result.df<- cbind(top1result.df, reference, species.cos, hits.cos, species.eu, hits.eu, species.ieu, hits.ieu)

# write.csv(top1result.df, file = "top1result_csd_sp_integer_neg_BT500.csv")

## If only data analysis from top1result dataframe shall be done, active lower command line.

# top1result.df<- read.csv(text=getURL("https://raw.github.com/Pohnert-Lab/SC-MS-Identification/master/Identification_results/Datafile_S10.csv", .opts=curlOptions(followlocation = TRUE))) # S10
...
# setEPS()
## ROC for identification of single microalgal cells with Cos/Eu/iEu on species level
...
# postscript("ROC_Scores_csd_sp_int_neg.eps", width = 8, height = 8)
...
## ROC for identification of single microalgal cells with Cos/Eu/iEu with bootstrapping assessment on species level
roc.cos.bt<-roc(top1result.df$hits.cos, top1result.df$bt.species.cos.score)
roc.eu.bt <-roc(top1result.df$hits.eu, top1result.df$bt.species.eu.score)
roc.ieu.bt<-roc(top1result.df$hits.ieu, top1result.df$bt.species.ieu.score)


# postscript("ROC_BT_csd_sp_int_neg.eps", width = 8, height = 8)
...
# postscript("SensErr_csd_Cos_sp_int_neg.eps", width = 8, height = 8)
...
err.cos$errorrates[min(which(err.cos$errorrates<=0.05))] # Errorrate
...
# postscript("SensErr_csd_Eu_sp_int_neg.eps", width = 8, height = 8)
...
err.eu$errorrates[min(which(err.eu$errorrates<=0.05))] # Errorrate
...
# postscript("SensErr_csd_iEu_sp_int_neg.eps", width = 8, height = 8)
...
err.ieu$errorrates[min(which(err.ieu$errorrates<=0.05))] # Errorrate
...
# postscript("SensErr_csd_BT_Cos_sp_int_neg.eps", width = 8, height = 8)
df.cos.bt<-data.frame(sc = top1result.df$bt.species.cos.score, hits = top1result.df$hits.cos)
...
err.cos.bt$errorrates[min(which(err.cos.bt$errorrates<=0.05))] # Errorrate
...
# postscript("SensErr_csd_BT_Eu_sp_int_neg.eps", width = 8, height = 8)
df.eu.bt<-data.frame(sc = top1result.df$bt.species.eu.score, hits = top1result.df$hits.eu)
...
err.eu.bt$errorrates[min(which(err.eu.bt$errorrates<=0.05))] # Errorrate
...
# postscript("SensErr_csd_BT_iEu_sp_int_neg.eps", width = 8, height = 8)
df.ieu.bt<-data.frame(sc = top1result.df$bt.species.ieu.score, hits = top1result.df$hits.ieu)
...
err.ieu.bt$errorrates[min(which(err.ieu.bt$errorrates<=0.05))] # Errorrate
...
for (i in c("cos","eu","ieu")){
  ...
  plot (p)
   ...
   # ggsave(filename = paste0("confmat_csd_sp_int_neg_",i,".eps"),device = "eps", width = 20, height = 20, units = "cm")
