...
# write.csv(top1result.df, file = "top1result_genus_integer_negative_BT500.csv")

## If only data analysis from top1result dataframe shall be done, active lower command line.

# top1result.df<- read.csv(text=getURL("https://raw.github.com/Pohnert-Lab/SC-MS-Identification/master/Identification_results/Dataset_S6.csv", .opts=curlOptions(followlocation = TRUE))) # URL S6
...
#postscript("ROC_Scores_gen_int_negative.eps", width = 8, height = 8)
...
#postscript("ROC_BT_gen_int_negative.eps", width = 8, height = 8)
...
#postscript("ErrSens_Cos_gen_int_negative.eps", width = 8, height = 8)
...
abline( v = err.cos$scores[min(which(err.cos$errorrates<=0.10))])
err.cos$scores[min(which(err.cos$errorrates<=0.10))] # Threshold
err.cos$errorrates[min(which(err.cos$errorrates<=0.10))] # Errorerate
sens.cos$sensitivity[which(sens.cos$scores == err.cos$scores[min(which(err.cos$errorrates<=0.10))])] # Sensitivity 
#dev.off()

## Score vs. Sensitivity/Errorrate plots
#Eu

#postscript("ErrSens_Eu_gen_int_negative.eps", width = 8, height = 8)
...
err.eu$errorrates[min(which(err.eu$errorrates<=0.1))] # Errorerate
...
#postscript("ErrSens_iEu_gen_int_negative.eps", width = 8, height = 8)
...
err.ieu$errorrates[min(which(err.ieu$errorrates<=0.1))] # Errorerate
...
#postscript("ErrSens_BT_Cos_gen_int_negative.eps", width = 8, height = 8)
...
err.cos.bt$errorrates[min(which(err.cos.bt$errorrates<=0.1))] # Errorerate
...
#postscript("ErrSens_BT_Eu_gen_int_negative.eps", width = 8, height = 8)
...
err.eu.bt$errorrates[min(which(err.eu.bt$errorrates<=0.1))] # Errorerate
...
#postscript("ErrSens_BT_iEu_gen_int_negative.eps", width = 8, height = 8)
...
err.ieu.bt$errorrates[min(which(err.ieu.bt$errorrates<=0.1))] # Errorerate
...
for (i in c("cos","eu","ieu")){
  ...
  #ggsave(filename = paste0("confmat_negative_int_",i,".eps"),device = "eps", width = 20, height = 20, units = "cm")
}
